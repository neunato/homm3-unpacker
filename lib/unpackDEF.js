...
const { defineDEFGroup } = require("./types")
...
function unpackDEF(buffer, options = {}) {
   ...
   for (let i = 0; i < groupCount; i++) {
      ...
      options.selection = group === "mouse over" || group === "standing"
      ...
      const { at } = buffer
      const sizes = offsets.map((offset) => {
         buffer.seek(offset)
         return buffer.uint32() + 32
      })
      buffer.seek(at)

      // Older DEF frames don't use `width`, `height`, `x`, `y` in header making them exceed buffer length by 16 bytes.
      options.legacy = sizes.some((size, i) => size + offsets[i] > buffer.length)
      ...
      groups[group] = buffer.loop(frameCount, (i) => {
         ...
         if (!images[name] || (options.selection && !images[name].selection))
...
function unpackFrame(buffer, { palette, legacy, selection, format, padding }) {
   ...
   if (legacy) {
      ...
      width = buffer.uint32()       // Cropped width.
      height = buffer.uint32()      // Cropped height.
      x = buffer.uint32()           // Offset left, relative to full width.
      y = buffer.uint32()           // Offset top, relative to full height.
   }
   ...
   if (padding) {
      ...
      bitmap = new BufferView(fullWidth * fullHeight * 4)  // All values initialised to 0, which is exactly what we use for transparency.
      ...
      if (!selection || !indices.includes(5))
         return image

      bitmap = new BufferView(fullWidth * fullHeight * 4)
      for (let i = 0; i < height; i++) {
         bitmap.seek((((y + i) * fullWidth) + x) * 4)
         for (let j = 0; j < width; j++) {
            const index = indices[(i * width) + j]
      ...
      image.selection = format === "bitmap" ? bitmap.buffer : upng.encode([bitmap.buffer], fullWidth, fullHeight, 0)
   }
   else {
      let bitmap

      bitmap = new BufferView(width * height * 4)
      ...
      image.selection = format === "bitmap" ? bitmap.buffer : upng.encode([bitmap.buffer], width, height, 0)
   }
